...
class OAuth2BasicAuthenticator < Auth::ManagedAuthenticator
  ...
  def after_authenticate(auth, existing_account: nil)
    ...
    if result && result.user && auth["credentials"] && auth["credentials"]["token"]
      ...
      log("OAuth2 Debugging: Setting current_access_token for user #{result.user.id} to: #{current_access_token}")
      result.user.custom_fields["current_access_token"] = current_access_token
      
      # Store refresh token if available
      if auth["credentials"]["refresh_token"].present?
        refresh_token = auth["credentials"]["refresh_token"]
        log("OAuth2 Debugging: Setting oauth2_refresh_token for user #{result.user.id}")
        result.user.custom_fields["oauth2_refresh_token"] = refresh_token
      end
      
      # Store token expiry if available
      if auth["credentials"]["expires_at"].present?
        expires_at = auth["credentials"]["expires_at"]
        log("OAuth2 Debugging: Setting oauth2_token_expires_at for user #{result.user.id} to: #{expires_at}")
        result.user.custom_fields["oauth2_token_expires_at"] = expires_at
      elsif auth["credentials"]["expires_in"].present?
        expires_at = Time.now.to_i + auth["credentials"]["expires_in"].to_i
        log("OAuth2 Debugging: Calculating oauth2_token_expires_at for user #{result.user.id} to: #{expires_at}")
        result.user.custom_fields["oauth2_token_expires_at"] = expires_at
      end
      
      result.user.save_custom_fields(true)
      log("OAuth2 Debugging: Saved token information for user #{result.user.id}")
    end
  ...
  def self.get_user_token(user)
    # Use the token manager to get a fresh token
    OAuth2TokenManager.get_token(user)
  end
end
