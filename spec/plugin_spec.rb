...
describe OAuth2BasicAuthenticator do
  ...
  describe "token refresh" do
    ...
    before do
      ...
      SiteSetting.oauth2_client_id = "client_id"
      SiteSetting.oauth2_client_secret = "client_secret"
      SiteSetting.oauth2_token_url = "https://provider.com/oauth2/token"
      SiteSetting.oauth2_authorize_url = "https://provider.com/oauth2/authorize"
      
      # Set up user with token information
      user.custom_fields["current_access_token"] = "old_access_token"
      user.custom_fields["oauth2_refresh_token"] = "refresh_token"
      user.custom_fields["oauth2_token_expires_at"] = Time.now.to_i - 100 # Expired token
      user.save_custom_fields(true)
    end
    
    it "stores refresh token and expiry during authentication" do
      ...
      expect(result.user.custom_fields["current_access_token"]).to eq("new_access_token")
      expect(result.user.custom_fields["oauth2_refresh_token"]).to eq("new_refresh_token")
      expect(result.user.custom_fields["oauth2_token_expires_at"].to_i).to be > Time.now.to_i
    end
    
    it "refreshes an expired token" do
      # Mock the token refresh
      refreshed_token = double(
        token: "refreshed_access_token",
        refresh_token: "new_refresh_token",
        expires_at: Time.now.to_i + 3600
      )
      
      access_token = double(refresh!: refreshed_token)
      
      # Mock the OAuth2::AccessToken.new call
      expect(OAuth2::AccessToken).to receive(:new).and_return(access_token)
      
      # Call the refresh method
      expect(OAuth2TokenManager.refresh_token(user)).to eq(true)
      
      # Reload user to get updated custom fields
      user.reload
      
      # Verify the token was updated
      expect(user.custom_fields["current_access_token"]).to eq("refreshed_access_token")
      expect(user.custom_fields["oauth2_refresh_token"]).to eq("new_refresh_token")
      expect(user.custom_fields["oauth2_token_expires_at"].to_i).to be > Time.now.to_i
    end
  end
end
